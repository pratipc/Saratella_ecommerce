...
def place_order():
    ...
    shipping_addr_str = data.get('shipping_address')
    billing_addr_str = data.get('billing_address')
    total_amount = data.get('total_amount')
    guest_email = data.get('guest_email')
    guest_phone = data.get('guest_phone')
    
    # NEW: Data for saving address
    save_new_shipping = data.get('save_new_shipping', False)
    shipping_components = data.get('shipping_components')
    
    connection = get_db_connection()
    try:
        cursor = connection.cursor(dictionary=True)
        
        # 1. Save New Shipping Address (if requested and user is logged in)
        if save_new_shipping and shipping_components and user_id:
            try:
                # Use AddUserAddress Stored Procedure
                cursor.callproc('AddUserAddress', [
                    user_id, 
                    'Shipping', 
                    shipping_components.get('line1'), 
                    '', # address_line2 is typically empty from this form
                    shipping_components.get('city'), 
                    shipping_components.get('state'), 
                    shipping_components.get('zip'), 
                    shipping_components.get('country')
                ])
                connection.commit()
            except Error as addr_err:
                print(f"DEBUG - Failed to save new address: {addr_err}")
                # We continue placing the order even if address save fails
        ...
        if order_info:
            # 3. Update Guest Email/Phone if provided
