...
def view_favorites():
    ...
    if not user_id:
        flash("Please login to view your favorite items.", "error")
    ...
    try:
        cursor = connection.cursor(dictionary=True)
        cursor.callproc('GetFavoriteProducts', [user_id, guest_id])
        for result in cursor.stored_results():
            favorites = result.fetchall()
            
        return render_template('user/favorites.html', user=session, products=favorites)
    except Error as e:
        print(f"DEBUG - Favorites Page Error: {e}")
        return render_template('user/favorites.html', user=session, products=[])
    finally:
        if connection and connection.is_connected():
            connection.close()


@user_bp.route('/api/user/favorite/toggle', methods=['POST'])
def toggle_favorite():
    user_id = session.get('user_id')
    if not user_id:
    ...
    try:
        cursor = connection.cursor(dictionary=True)
        cursor.callproc('ToggleFavoriteProduct', [user_id, product_id])
        is_favorite = 0
        for result in cursor.stored_results():
            row = result.fetchone()
            if row:
...
@user_bp.route('/api/search/config', methods=['GET'])
@user_bp.route('/search/config', methods=['GET'])
@user_bp.route('/api/user/search/config', methods=['GET'])
def search_config():
    connection = get_db_connection()
