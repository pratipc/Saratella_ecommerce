...
def dashboard():
    store_id = session.get('warehouse_id')
    ...
    if store_id:
        ...
        try:
            ...
            # SMART ROUTING: Fetch the right data based on role
            if user_type == 'warehouse_worker':
                cursor.callproc('GetWorkerDashboardStats', [store_id, user_id])
            else:
                cursor.callproc('GetWarehouseDashboardStats', [store_id])
            ...
            if user_type == 'warehouse_manager':
                cursor = connection.cursor(dictionary=True)
                cursor.callproc('GetPendingBackorders', [store_id])
...
def fulfillment_queue():
    ...
    if store_id:
        ...
        try:
            ...
            # Fetch the highly optimized queues
            cursor.callproc('GetWarehouseDashboardStats', [store_id])
            results = list(cursor.stored_results())
            ...
            cursor.nextset() # Clear out previous results
            cursor.callproc('GetWarehouseWorkers', [store_id])
            for result in cursor.stored_results():
    ...
    # Pass the heavy lifting directly to the template
    return render_template('warehouse/manager_queue.html', 
                           pending_queue=pending_queue, 
                           active_queue=active_queue,
                           staff=staff)

# ... NEW API ROUTES NEEDED FOR THE MASTER QUEUE ...
...
def api_bulk_release_orders():
    ...
    # Convert Python list to JSON string for the MySQL Stored Procedure
    order_ids_json = json.dumps(order_ids)

    connection = get_db_connection()
    try:
        cursor = connection.cursor()
        cursor.callproc('BulkReleaseOrders', [store_id, order_ids_json])
        connection.commit()
...
def assign_worker(order_id):
    ...
    store_id = session.get('warehouse_id')
    manager_id = session.get('user_id') # Get the MANAGER who is doing the assigning
    data = request.get_json()
    ...
    try:
        cursor = connection.cursor()
        # Pass manager_id to track who assigned it
        cursor.callproc('AssignWarehouseWorker', [order_id, store_id, worker_id, manager_id])
        connection.commit()
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def pick_and_pack():
    ...
    if store_id and worker_id:
        connection = get_db_connection()
        try:
            cursor = connection.cursor(dictionary=True)
            cursor.callproc('GetWorkerTasks', [store_id, worker_id])
...
def claim_task(order_id):
    ...
    try:
        cursor = connection.cursor()
        # Pass worker_id twice (assigned to self = Auto-Queue)
        cursor.callproc('AssignWarehouseWorker', [order_id, store_id, worker_id, worker_id])
        connection.commit()
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_add_staff():
    ...
    # App Layer Security: Hash the password before sending to DB
    ...
    try:
        ...
        # Strip out the MySQL error code (e.g. "1644 (45000): ") to show a clean message
...
def api_toggle_staff_status(worker_id):
    ...
    try:
        cursor = connection.cursor()
        cursor.callproc('ToggleWorkerStatus', [worker_id, store_id, is_active])
        connection.commit()
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_dashboard_stats():
    ...
    if not store_id:
        return jsonify({"error": "No warehouse assigned."}), 400
...
def get_order_details(order_id):
    store_id = session.get('warehouse_id')
    print(f"DEBUG [Packing Slip]: Attempting to fetch Order {order_id} for Store {store_id}")
    
    connection = get_db_connection()
    try:
        cursor = connection.cursor(dictionary=True)
        cursor.callproc('GetWarehouseOrderDetails', [order_id, store_id])
        ...
        if not order_header:
            print(f"DEBUG [Packing Slip]: FAILED. DB returned 0 rows. Is the SQL Procedure missing?")
            return jsonify({"error": f"Order {order_id} not found or you don't have access."}), 404
            
        print(f"DEBUG [Packing Slip]: SUCCESS. Returning order {order_header['order_number']}")
        return jsonify({"order": order_header, "items": order_items})
    except Exception as e:
        print(f"DEBUG [Packing Slip]: PYTHON/SQL CRASH: {e}")
        return jsonify({"error": str(e)}), 500
    finally:
        if connection and connection.is_connected():
            cursor.close()
...
def update_order_status(order_id):
    store_id = session.get('warehouse_id')
    worker_id = session.get('user_id') # Grab the worker ID to log exceptions
    data = request.get_json(silent=True) or {}
    
    new_status = data.get('status')
    picked_items = data.get('picked_items', []) # The array of items they actually packed
    
    # We now allow 'partially_shipped'
    if new_status not in ['processing', 'shipped', 'partially_shipped', 'cancelled']:
        return jsonify({"error": "Invalid status update."}), 400
        
    # Convert the Python list back to a JSON string so MySQL can loop through it
    items_json_str = json.dumps(picked_items) if picked_items else None
        
    connection = get_db_connection()
    try:
        cursor = connection.cursor()
        # Call our new Enterprise Engine WITH the worker_id parameter
        cursor.callproc('ProcessOrderFulfillment', [order_id, store_id, new_status, items_json_str, worker_id])
        connection.commit()
...
def api_get_exceptions():
    ...
    try:
        cursor = connection.cursor(dictionary=True)
        cursor.callproc('GetWarehouseExceptions', [store_id])
...
def api_resolve_exception(exception_id):
    ...
    # Safely convert to int, handling empty strings or None
...
def api_get_exceptions_history():
    ...
    try:
        cursor = connection.cursor(dictionary=True)
        cursor.callproc('GetResolvedWarehouseExceptions', [store_id])
    ...
    # RENDER THE HTML TEMPLATE DIRECTLY, PASSING THE DATA
...
def release_backorder(backorder_id):
    ...
    try:
        cursor = connection.cursor()
        cursor.callproc('ReleaseBackorderToFloor', [backorder_id, store_id])
        connection.commit()
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
